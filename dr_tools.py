...
lastmodified = "2 Sep 2019"

class BufferedOutfile:
	# creates a temporary file for writing, then moves that file to the output filename, or deletes the temporary file if an exception is raised
	def __init__(self, outfile, mode='w', tmp_suffix=''):
		self.fh = None # filehandle, use this for writing to
		self.tmp_outf = None # filename, to use for writing to also set mode=None
		self.final_outf = outfile #internal
		self.tmp_suffix = tmp_suffix #internal
		self.mode = mode #internal
	def __enter__(self):
		import tempfile, os
		
		# make sure it's the right file system
		tmpfolder = tempfile.gettempdir()
		outfolder = os.path.dirname(os.path.abspath(self.final_outf))
		if os.lstat(tmpfolder).st_dev != os.lstat(outfolder).st_dev:
			tmpfolder = outfolder
		
		# create and open the temporary file
		self.tmp_outf = tempfile.mkstemp(self.tmp_suffix, dir=tmpfolder)[1]
		if self.mode is not None: self.fh = open(self.tmp_outf, self.mode)
		return self
	def __exit__(self, exc_type, exc_value, exc_traceback):
		import os
		if self.fh is not None: self.fh.close()
		try:
			if exc_type is None: # successful
				os.rename(self.tmp_outf, self.final_outf)
		except IOError:
			os.remove(self.tmp_outf)
			raise
...
def exprfromdataframe(data, is_counts=False):
    values = Parsed_rpkms(['exprfromdataframe'], is_counts)
    values.samples = data.columns
    values['symbols'] = list(data.index)
    values['IDs'] = ['' for sym in values['symbols']]
    values.symbol_to_index = dict((S, i) for i, S in enumerate(values['symbols']))
    for col in values.samples:
        values[col] = list(data[col])
    return values
...
class Cregion:
	...
	def __hash__(self):
		return self.chrindex, self.start, self.end
